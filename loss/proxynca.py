...
class Criterion(nn.Module):
    def __init__(self, opt):
        ...
        self.proxies = nn.Parameter(torch.randn(self.num_proxies, self.embed_dim) / 8)
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        batch = 3 * nn.functional.normalize(batch, dim=1)
        proxies = 3 * nn.functional.normalize(self.proxies, dim=1)
