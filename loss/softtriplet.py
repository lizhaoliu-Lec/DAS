...
class Criterion(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.intra_class_centroids = nn.Parameter(torch.Tensor(opt.embed_dim, self.n_classes * self.n_centroids))
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        intra_class_centroids = F.normalize(self.intra_class_centroids, dim=1)
        similarities_to_centroids = batch.mm(intra_class_centroids).reshape(-1, self.n_classes, self.n_centroids)

        soft_weight_over_centroids = nn.Softmax(dim=1)(self.gamma * similarities_to_centroids)
        ...
        centroid_classification_loss = nn.CrossEntropyLoss()(self.lam * (per_class_embed - margin_delta),
