...
import datasampler.super_class_random_sampler
...
def select(sampler, opt, image_dict, image_list=None, **kwargs):
    ...
    if 'batchmatch' in sampler:
        ...
        if 'super' in sampler:
            sampler_lib = super_class_random_sampler
        elif 'class' in sampler:
    ...
    sampler = sampler_lib.Sampler(opt, image_dict=image_dict, image_list=image_list, **kwargs)
