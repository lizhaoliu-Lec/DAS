    ...
    def __call__(self, features, target_labels):
        ...
        if 'intra' in self.mode:
            if isinstance(features, torch.Tensor):
                intra_features = features.detach().cpu().numpy()
            else:
                intra_features = features

            intra_dists = []
            for loc in features_locs:
                c_dists = distance.cdist(intra_features[loc], intra_features[loc], 'cosine')
                c_dists = np.sum(c_dists) / (len(c_dists) ** 2 - len(c_dists))
                intra_dists.append(c_dists)
            intra_dists = np.array(intra_dists)
            max_val = np.max(intra_dists[1 - np.isnan(intra_dists)])
            intra_dists[np.isnan(intra_dists)] = max_val
            intra_dists[np.isinf(intra_dists)] = max_val
        ...
        if 'inter' in self.mode:
            if not isinstance(features, torch.Tensor):
                ...
                coms = []
                for loc in features_locs:
                    com = F.normalize(torch.mean(features[loc], dim=0).reshape(1, -1),
                                      dim=-1).reshape(1, -1)
                    coms.append(com)
